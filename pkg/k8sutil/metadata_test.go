...
func TestUpdateNodeRetryHandlesConflict(t *testing.T) {
	...
	mockNi.EXPECT().Get(gomock.Any(), "mock_node", v1meta.GetOptions{}).Return(mockNode, nil).AnyTimes()

	// Conflict once; mock that a third party incremented the counter from '20'
	// to '21' right after the node is returned
	gomock.InOrder(
		mockNi.EXPECT().Update(gomock.Any(), gomock.Any(), gomock.Any()).Do(func(ctx context.Context, n *v1api.Node, opts v1meta.UpdateOptions) {
		...
		mockNi.EXPECT().Update(gomock.Any(), gomock.Any(), gomock.Any()).Return(mockNode, nil),
	)
